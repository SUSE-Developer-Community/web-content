# Cluster, This is Your Admin - Do You Read?
...
The most widely used mechanism to find cluster access information are so-called _kubeconfig files_. By default, kubectl will use information from the file `$HOME/.kube/config`. It is possible to point to other config files by setting the `KUBECONFIG` environment variable or by using the `--kubeconfig` flag. 

There are three main components in a kubeconfig file:
...
* The content of all configs specified in $KUBECONFIG is merged. You can use `kubectl config view --merge` to check what this leads to on your system. 
...
So far we've obviously only covered Kubernetes' built-in functionality. The are a couple of little helpers and additional tricks that are worth knowing about. Disclaimer: this is not intended to be a complete list - there's tons of other stuff out there. Just sharing a few of my favorites. 

The first tool I want to call out is Ahmet Balkan's kubectx and its brother-in-arms, kubens. Both are available from the same [GitHub repo](https://github.com/ahmetb/kubectx). They give you a shorter way to switch between contexts and namespaces than their somewhat lengthy kubectl counterparts. Plus, they basically behave like the cd command for changing directories (e.g. `kubectx -` gets you back to the previous context). Nothing magic, but neat. Check the embedded videos in the GitHub readme, and you'll quickly get the idea. 

Another trick many Kubernetes people use is to display current cluster and namespace your command line prompt. You might think this is unnecessary, but wait until you accidentally delete a pod from the wrong namespace. If you are using bash or zsh (the default shell on MacOS), you can simply use [https://github.com/jonmosco/kube-ps1](https://github.com/jonmosco/kube-ps1). You can install it via your favorite package manager, and you'll have to add a few lines to your .bashrc or .zshrc. The repository's readme describes the process well. If you're using [fish](https://fishshell.com), you may want to take a look at [https://github.com/aluxian/fish-kube-prompt] for the same purpose (tip: have a look at [fisher](https://github.com/jorgebucaran/fisher) to manage your fish extensions). You can easily turn both prompt mods on and off with `kubeon|kubeoff` for kube-ps1 and `kube_ps on|off` for fish-kube-prompt. 
